...
1. **회원**
    ...
    - 유료 정책
        ...
        - 결제 이후 새로 등록된 와치리스트가 생성되면 이용했다는 것으로 간주되어 환불불가.
        - 기존 플랜 이용자가 동시에 새로운 플랜을 추가로 결제한경우 상위 등급 플랜을 먼저 적용받고 기존 플랜은 홀딩된 상태여야함
        - 플랜 기간이 끝나 하위 플랜으로 변경될경우 1주일의 계도기간을 거쳐 하위플랜을 벗어나는 최근에 추가된 와치리스트들을 결제 필요 상태로 바꿔야함.
        - 결제 필요 상태는 일시정지 상태와 다르게 사용자 임의대로 풀 수 없고 결제를 해야 풀어진다. 결제가 확인되면 일시정시 상태로 변경된다.
