...
# 예시: ./deploy.sh backend dev ghcr.io/byeongminbae/watchtower-backend-dev:latest
# EC2에서 이미 `docker login ghcr.io` 되어 있어야만 작동됨
...
if [[ "${V1_RUNNING}" == false && "${V2_RUNNING}" == false ]]; then
  ...
  if health_check "${NEW_PORT}"; then
    ...
    docker stop --time=60 "${OLD_NAME}" >/dev/null 2>&1 || true
    ...
    docker stop --time=60 "${NEW_NAME}" >/dev/null 2>&1 || true
